...
function setTransitionHooks$1(vnode, hooks) {
  if (vnode.shapeFlag & 6 && vnode.component) {
    if (isVaporComponent$1(vnode.type)) {
...
function activate$1(vnode, container, anchor, { p: patch, m: move }, parentComponent, parentSuspense, namespace, optimized) {
...
function deactivate$1(vnode, container, { m: move }, parentComponent, parentSuspense) {
  const instance = vnode.component;
...
function patchEvent(el, rawName, prevValue, nextValue, instance = null) {
  ...
  if (nextValue && existingInvoker) {
    ...
    if (nextValue) {
      const invoker = invokers[rawName] = createInvoker$1(
        nextValue,
        instance
      );
...
const VaporTransition = /* @__PURE__ */ decorate$1((props, { slots }) => {
  ...
  const performAppear = isHydrating ? hydrateTransitionImpl() : void 0;
  const children = slots.default && slots.default();
  if (!children) return [];
  const instance = currentInstance;
  const { mode } = props;
  ...
  if (block instanceof Node) {
    ...
    if (isAsyncWrapper(block)) {
      if (!block.type.__asyncResolved) {
        onFragment && onFragment(block.block);
      } else {
  ...
  setup(props, { slots }) {
    ...
    const cacheBlock = () => {
      ...
      if (isDynamicFragment(block)) {
        ...
        if (transition && transition.mode === "out-in" && transition.state.isLeaving) {
          return;
        }
      }
...
const vaporInteropImpl = {
  ...
  hydrate(vnode, node, container, anchor, parentComponent, parentSuspense) {
    if (!isHydrating && !isHydrating$1) return node;
    hydrateNode(
      node,
...
function createIf(condition, b1, b2, once) {
  ...
  let frag;
  if (once) {
    frag = condition() ? b1() : b2 ? b2() : [createTextNode()];
  } else {
...
function createKeyedFragment(key, render) {
  ...
  const _isLastInsertion = isLastInsertion;
  if (!isHydrating) resetInsertionState();
  const frag = new DynamicFragment(void 0, true);

...
const show = ref(true)
  ...
  const n0 = _createIf(() => (show.value), () => {
    const n2 = t0()
    const x2 = _txt(n2)
    _renderEffect(() => _setText(x2, _toDisplayString(msg.value) + " " + _toDisplayString(__props.item)))
    return n2
  }, () => {
    const n4 = t1()
    return n4
  ...
  const n6 = _createFor(() => (5), (_for_item0) => {
    const n8 = t2()
    const x8 = _txt(n8)
    _renderEffect(() => _setText(x8, _toDisplayString(_for_item0.value)))
    return n8
  }, undefined, 4)
  _setInsertionState(n10, 4)
  const n9 = _createSlot("default", null)
  _applyTextModel(n5, () => (msg.value), _value => (msg.value = _value))
  return n10
